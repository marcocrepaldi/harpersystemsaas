...
type InsurerListResp = {
  items: Array<{ id: string; tradeName: string; legalName: string }>;
  page: number;
  limit: number;
  total: number;
};
...
export default function ReconciliationBoard() {
  ...
  // --- estado: mês + seguradora (URL-driven) ---
  const mesParam = sp.get('mes') || '';
  const insurerParam = sp.get('insurerId'); // pode ser null

  const defaultYm = React.useMemo(() => dateToYm(new Date()), []);
  const [mes, setMes] = React.useState<string>(mesParam || defaultYm);
  const [insurerId, setInsurerId] = React.useState<string | null>(insurerParam ?? null);

  // inicializa ?mes na URL se não veio
  React.useEffect(() => {
    if (!mesParam) {
  ...
  // mantém estado em sincronia com URL externa (navegação/back)
  React.useEffect(() => {
  ...
  React.useEffect(() => {
    if ((insurerParam ?? null) !== insurerId) setInsurerId(insurerParam ?? null);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [insurerParam]);

  const updateUrl = React.useCallback(
    (next: { mes?: string; insurerId?: string | null }) => {
      const params = new URLSearchParams(window.location.search);
      const _mes = next.mes ?? mes;
      const _ins = next.insurerId === undefined ? insurerId : next.insurerId;

      params.set('mes', _mes);
      if (_ins) params.set('insurerId', _ins);
  ...
  const handleInsurerChange = React.useCallback(
    (v: string | null) => {
      setInsurerId(v);
      updateUrl({ insurerId: v });
    },
    [updateUrl],
  );
  ...
  // limpa seleções ao mudar filtros-chave
  React.useEffect(() => {
    setSelectedInvoiceIds([]);
  }, [activeTab, mes, tipo, plano, centro, insurerId, debouncedSearch]);

  // === Seguradoras (ativas, linha HEALTH) ===
  const { data: insurersResp } = useQuery<InsurerListResp>({
    queryKey: ['insurers', 'HEALTH', clienteId],
    queryFn: () =>
      apiFetch<InsurerListResp>(
        `/insurers?line=HEALTH&isActive=true&limit=200&sortBy=tradeName&sortOrder=asc`,
      ),
  });

  const insurers = React.useMemo(
    () =>
      (insurersResp?.items || []).map((i) => ({
        id: i.id,
        name: i.tradeName || i.legalName,
        tradeName: i.tradeName,
      })),
    [insurersResp],
  );

  // === Options (planos/centros) — backend ignora params extras, mas enviamos insurerId por consistência ===
  const { data: options } = useQuery<OptionsResp>({
    queryKey: ['recon.options', clienteId, insurerId],
    queryFn: () =>
      apiFetch<OptionsResp>(
        `/clients/${clienteId}/reconciliation/options${
          insurerId ? `?insurerId=${encodeURIComponent(insurerId)}` : ''
        }`,
      ),
  });
  ...
  const { data, isLoading, isError, refetch } = useQuery<ReconResp>({
    queryKey: ['recon.v1', clienteId, mes, tipo, plano, centro, insurerId],
    queryFn: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      ...
      // OBS.: endpoint legado não segmenta por seguradora (mantido p/ compatibilidade)
  ...
  // 🔒 Fechamento manual (mantendo nomes LEGADOS + insurerId opcional)
  const closeMutation = useMutation({
    mutationFn: async ({ total, notes }: { total: number; notes?: string }) => {
      const body: any = {
        mes,
        valorFaturaDeclarado: total,     // nomes legados mantidos
        observacaoFechamento: notes?.trim() || undefined,
      };
      if (insurerId) body.insurerId = insurerId; // novo campo opcional
